...
export default function CodeContent({ activeSection }: CodeContentProps) {
  ...
  return (
    <div className={styles.code}>
      {content && content.length > 0 ? content.map((line, index) => (
        <div key={index} className={styles.codeLine}>
          {line && (
            <>
              <span className={line.class && styles[line.class] ? styles[line.class] : ''}>{line.text || ''}</span>
